...
class MqDirect2 extends Command
{
    protected function configure()
    {
        // 指令配置
        $this->setName('direct_msg2')
            ->setDescription('the direct_msg2 command');
    }
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        //绑定交换机与队列，并指定路由键
        $channel->queue_bind($queue, $exchange_name, 'info');
        $channel->queue_bind($queue, $exchange_name, 'error');
        $channel->queue_bind($queue, $exchange_name, 'warning');
