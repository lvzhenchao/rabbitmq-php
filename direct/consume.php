<?php
require_once '../vendor/autoload.php';
use PhpAmqpLib\Connection\AMQPStreamConnection;

//数据库名称
$v_host = "order";

//交换器名称
$exchange_name = 'direct_log';

//指定routing_key
$routing_key = "info";

//创建连接
$connection = new AMQPStreamConnection("127.0.0.1", "5672", "lzc", "lzc", $v_host);

//创建连接
$channel = $connection->channel();

//声明交换器
$channel->exchange_declare($exchange_name, 'direct', false, false, false);//第二个参数：交换器类型：fanout、direct、topic、headers

//获取消息队列 名称
//$queue_name_arr =  $channel->queue_declare("", false, false, true, false);
//print_r($queue_name_arr);
//Array
//(
//    [0] => amq.gen-pPkE6-2I4cidq51n6ZTRNQ //随机队列名称
//    [1] => 0
//    [2] => 0
//)
list($queue_name,,) = $channel->queue_declare("", false, false, true, false);

//将交换器上获取的随机名称绑定到 消息队列上
$channel->queue_bind($queue_name, $exchange_name, $routing_key);//绑定routing Key

//更改并发数量
$channel->basic_qos(null, 1, null);

//取数据：消费数据
$callback = function ($msg) {
//    print_r($msg);
    echo 'received: ',$msg->body,"\n";
};

//消费
$channel->basic_consume($queue_name, '', false, false, false, false, $callback);


//监控
while ($channel->is_open()) {
    $channel->wait();
}

$channel->close();

$connection->close();
